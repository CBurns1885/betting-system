...
class RealisticBacktest:
    ...
    def __init__(self,
        ...
        # Backup existing weekly_bets_full.csv
    ...
    def backup_existing_outputs(self):
        """Backup existing weekly_bets_full.csv before backtesting"""
        try:
            weekly_bets_full = OUTPUT_DIR / "weekly_bets_full.csv"
            if weekly_bets_full.exists():
                timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
                backup_path = OUTPUT_DIR / f"weekly_bets_full_backup_{timestamp}.csv"
                shutil.copy(weekly_bets_full, backup_path)
                print(f"✅ Backed up existing weekly_bets_full.csv to {backup_path.name}")
        except Exception as e:
            print(f"⚠️ Could not backup weekly_bets_full.csv: {e}")
    ...
    def restore_outputs(self):
        ...
        try:
            ...
            # Remove weekly_bets_full.csv created during backtest
            weekly_bets_full = OUTPUT_DIR / "weekly_bets_full.csv"
            if weekly_bets_full.exists():
                # Check if it's a backtest file by seeing if backtest_outputs exists
                latest_backup = None
                for backup in OUTPUT_DIR.glob("weekly_bets_full_backup_*.csv"):
                    if latest_backup is None or backup.stat().st_mtime > latest_backup.stat().st_mtime:
                        latest_backup = backup

                if latest_backup:
                    shutil.copy(latest_backup, weekly_bets_full)
                    print(f"✅ Restored original weekly_bets_full.csv from backup")
                else:
                    weekly_bets_full.unlink()
                    print(f"✅ Removed backtest weekly_bets_full.csv")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\n✅ Your original weekly_bets_full.csv was preserved!")
